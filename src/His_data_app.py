...
def cargar_csv():
    ...
    try:
        ...
        messagebox.showinfo("Éxito", f"Archivo '{file_path}' cargado correctamente")
    except Exception as e:
        messagebox.showerror("Error", f"No se pudo cargar el archivo:\n{e}")

def generar_grafico():
    if df is None:
        messagebox.showwarning("Sin archivo", "Primero carga un archivo CSV")
        return
    ...
    canvas_fig.draw()
    canvas_fig.get_tk_widget().pack(anchor="center")  # centrado
    ...
    canvas_tabla.draw()
    canvas_tabla.get_tk_widget().pack(anchor="center")  # centrado
...
tk.Button(root, text="Cargar CSV", command=cargar_csv).pack(pady=5)

columna_var = tk.StringVar(value="Actual")
tk.Label(root, text="Selecciona la columna:").pack()
...
tk.Label(root, text="Selecciona el tipo de valor:").pack()
